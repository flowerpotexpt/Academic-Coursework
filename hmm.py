...
"""
DESCRIPTION:
    Hidden Markov Models assignment in the Algorithms in Sequence Analysis course at the VU.

AUTHOR:
    <Yashjit Gangopadhyay> <2662062>
"""
...
def baumwelch(set_X, A, E):
    ...
    for X in set_X:
        ...
        for k in allStates:
            new_A[k]["E"] += B[k][-2] * F[k][(len(X)+2)-2] / P

        for k in allStates:
            for l in emittingStates:
                for i in range((len(X)+2) - 2):
                    s = X[i]
                    new_A[k][l] += B[l][i+1] * A[k][l] * E[l][s] * F[k][i] / P
    ...
    for k_1 in allStates:
        total_row_a = sum(new_A[k_1].values())
        if total_row_a != 0:
            for k_2 in allStates:
                new_A[k_1][k_2] /= total_row_a

    for l in emittingStates:
        total_row_e = sum(new_E[l].values())
        for s in new_E[l].keys():
            new_E[l][s] /= total_row_e
